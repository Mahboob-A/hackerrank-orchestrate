...
- **`code/main.py`**: CLI batch runner coordinating ingestion $\rightarrow$ evaluation $\rightarrow$ formatting into `output.csv`.
...
5. **Minimum Balance Floor:** $\text{balance}(t) \ge \text{minimum\_balance\_to\_keep}$ on **every single day** $t \in [0, 90]$.
6. **Amount Safe to Pay:** Unassisted headroom on `request_date`, bounded by $[0, \text{requested\_amount}]$.
...
8. **Partial Payment Rules:**
   - Exactly 2 payments: $P_1 = \text{amount\_safe\_to\_pay}$ on `request_date`, $P_2 = \text{remainder}$ on `earliest_date_for_full_payment`.
   - $P_1 + P_2 \equiv \text{requested\_amount}$.
   - Allowed only if $0 < P_1 < \text{requested\_amount}$ AND $\text{earliest\_date} \le \text{desired\_completion\_date}$.
